...
DRIVER_FAMILY_PROMPT_VERSION = "1.0.0"
DRIVER_FAMILY_COMPILER_VERSION = "1.0.0"
DEFAULT_MAX_FAMILY_PROJECTION_CHARS = 120_000
...
snapshot. Explain the conditions for each case and what evidence would change the view.
...
and required capability; never invent a numeric proxy. Return only the requested structured
critique contract."""
...
structured family contract, not a patch."""
...
def _family_analysis_projection(
    ...
    *,
    ...
    for component in family_components:
        projection[component] = getattr(snapshot, component)
...
def run_driver_family_workflow(
    *,
    ...
    max_projection_chars: int = DEFAULT_MAX_FAMILY_PROJECTION_CHARS,
    ...
    try:
        analysis_projection = _family_analysis_projection(
            snapshot,
            selected_family,
            max_chars=max_projection_chars,
        )
